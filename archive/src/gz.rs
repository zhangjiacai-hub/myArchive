...
pub fn compress_file(source: &str, output: &str) -> Result<()> {
    ...
    if !source_path.is_file() {
        anyhow::bail!("gz 只支持单文件压缩，目录请使用 tar.gz 格式: {}", source);
    }
...
pub fn decompress_file(source: &str, output: &str) -> Result<()> {
    let input = File::open(source).with_context(|| format!("无法打开 gz 文件: {}", source))?;
...
pub fn extract_tar(source: &str, output: &str) -> Result<()> {
    let file = File::open(source).with_context(|| format!("无法打开 tar.gz 文件: {}", source))?;
